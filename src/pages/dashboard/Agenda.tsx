...
import { format, startOfDay, endOfDay, addDays, startOfWeek, endOfWeek, isSameDay } from 'date-fns';
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={goToday} className="transition-all duration-200 hover:scale-[1.02] active:scale-[0.97]">
            Hoje
          </Button>
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" onClick={prevDay} className="transition-all duration-200 hover:scale-[1.05] active:scale-[0.95]">
            ...
            <Button variant="ghost" size="icon" onClick={nextDay} className="transition-all duration-200 hover:scale-[1.05] active:scale-[0.95]">
      ...
      {professionals.length === 0 ? (
        ...
        <DayView
          ...
          hours={HOURS}
          getAptsForCell={getAptsForCell}
          onClickApt={setDetailApt}
        />
      ) : (
        <WeekView
          professionals={professionals}
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 flex-wrap">
                {detailApt.status === 'pending' && (
                  <Button size="sm" onClick={() => updateStatus.mutate({ id: detailApt.id, status: 'confirmed' })} className="transition-all duration-200 hover:scale-[1.02] active:scale-[0.97]">
                    Confirmar
                  </Button>
                )}
                {(detailApt.status === 'pending' || detailApt.status === 'confirmed') && (
                  <>
                    <Button size="sm" variant="secondary" onClick={() => updateStatus.mutate({ id: detailApt.id, status: 'completed' })} className="transition-all duration-200 hover:scale-[1.02] active:scale-[0.97]">
                      Concluir
                    </Button>
                    <Button size="sm" variant="destructive" onClick={() => updateStatus.mutate({ id: detailApt.id, status: 'cancelled' })} className="transition-all duration-200 hover:scale-[1.02] active:scale-[0.97]">
                      Cancelar
                    </Button>
                  </>
                )}
...
function WeekView({ professionals, weekDays, hours, getAptsForCell, onClickApt }: any) {
  // Simplified week: show all appointments per day without hour rows
  return (
    <div className="space-y-4">
